...
/* 如上个步骤的代码中，ServiceDelegateDAO 类对其注入的dao进行委派，而dao一共有104个方法，那么ServiceDelegateDAO自己就需要设计104个方法，假如都手动写会非常耗时，而且容易出错。有什么办法可以又快又准地生成委派代码呢？
答案是有的，我们可以借助eclipse的快速生成委派模式来解决这个问题。
1. 首先回到ServiceDelegateDAO 只有dao属性的状态，并且选中dao属性，一定要选中哦
2. 然后点击菜单->Source->Generate Delegate Methods -> Select All -> OK
这样就快速而不出错地对dao的104个方法自动生成了委派方法。 */
